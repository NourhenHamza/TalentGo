...
class NotificationAPI {
   ...
   * @returns {string|null} Jeton d'authentification
   */
  getAuthToken() {
    const token = localStorage.getItem('token');
    console.log('Retrieved token from localStorage:', token ? token.substring(0, 10) + '...' : 'No token');
    return token || null;
  }
  ...
  async request(url, options = {}, retries = 3, delay = 5000) {
    ...
    console.log('API Request - URL:', url, 'Token:', token ? token.substring(0, 10) + '...' : 'No token');
    ...
    const config = {
      ...options,
      headers
      // Removed credentials: 'include' since we're not using cookies
    };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          console.error('Unauthorized access, clearing localStorage');
