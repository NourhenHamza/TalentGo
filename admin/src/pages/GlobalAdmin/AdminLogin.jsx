...
const AdminLogin = () => {
  ...
  // Use the loginAdmin function from context instead of making separate axios call
  const { bToken, setbToken, backendUrl, loginAdmin } = useContext(GlobalAdminContext)
  const navigate = useNavigate()

  // Debug: Check localStorage on component mount
  useEffect(() => {
    console.log("Component mounted - Current localStorage bToken:", localStorage.getItem('bToken'))
    console.log("Component mounted - Current bToken from context:", bToken)
  }, [bToken])
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      // Use the context's loginAdmin function instead of direct axios call
      const result = await loginAdmin({ email, password })
      
      if (result && result.success) {
        console.log("Login successful via context")
        toast.success("Admin logged in successfully!")
        navigate("/globaladmin-dashboard")
      } else {
        console.error("Login failed via context")
        // Error message already shown by context's loginAdmin function
      }
    } catch (error) {
          ...
          <div className="grid md:grid-cols-5 gap-8 items-center">
            <div className="md:col-span-2 text-center md:text-left">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                <div className="mb-6">
                  ...
                  <h1 className="text-3xl md:text-4xl font-bold text-slate-800 mb-2">Admin Portal</h1>
                  <p className="text-slate-500 text-lg">PFE System Administration</p>
                </div>
                ...
                <div className="hidden md:block">
                  <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl shadow-xl p-8 text-white relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                      ...
                      <p className="text-blue-50 mb-6">Manage the entire Pfe Platform from a centralized dashboard</p>
                    </div>
                  </div>
                </div>
              </motion.div>
            ...
            >
              <div className="bg-white rounded-3xl shadow-xl p-8 md:p-10 border border-slate-100 relative">
                <div className="mb-8">
                  <h2 className="text-2xl font-bold text-slate-800 mb-2">Global Admin Authentication</h2>
                  <p className="text-slate-500">Sign in to access the administration dashboard</p>
                </div>
