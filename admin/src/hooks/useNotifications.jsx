...
export const useNotifications = (options = {}) => {
  ...
  // Get the active token based on priority (customize as needed)
  const getActiveToken = useCallback(() => {
    const tokens = { aToken, uToken, bToken, cToken, dToken, eToken, rToken };
    console.log('Available tokens:', tokens);
    return aToken || uToken || bToken || cToken || dToken || eToken || rToken || localStorage.getItem('token');
  }, [aToken, uToken, bToken, cToken, dToken, eToken, rToken]);
  ...
  const loadNotifications = useCallback(async (page = 1, reset = false) => {
    try {
      ...
      console.log('Token from contexts:', token ? token.substring(0, 10) + '...' : 'No token');
      if (!token) {
  ...
  }, [filters, pageSize, getActiveToken]);
  ...
  const updateUnreadCount = useCallback(async () => {
    try {
      const token = getActiveToken();
      if (!token) {
        console.error('❌ Aucun token disponible pour updateUnreadCount');
        return;
      }
  ...
  }, [enableRealTime, addNotification, updateNotification, getActiveToken]);
